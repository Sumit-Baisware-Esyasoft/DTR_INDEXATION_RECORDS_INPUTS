...
st.markdown("""
    ...
    <p style='text-align: center; color: gray; font-size: 18px;'>
        कृपया नीचे दी गई सभी जानकारी ध्यानपूर्वक भरें।
    </p>
    <hr style='margin-top: 10px; margin-bottom: 20px;'>
""", unsafe_allow_html=True)
...
# ----------------- LOAD HIERARCHY -----------------
st.subheader("📂 हाइरार्की विवरण भरें")
...
if msn_auto:
    st.markdown("### ✅ MSN की पुष्टि करें")
    st.info(f"🔍 चुना गया MSN: **{msn_auto}**")
    confirm = st.radio("क्या यह MSN सही है?", ["हाँ, सही है ✅", "नहीं, बदलना है ❌"], horizontal=True)

    if confirm == "हाँ, सही है ✅":
        final_msn = msn_auto
    else:
        new_msn = st.text_input("✏️ नया MSN दर्ज करें")
        if new_msn:
            final_msn = new_msn
