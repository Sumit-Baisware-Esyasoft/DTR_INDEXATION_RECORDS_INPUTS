...
try:
    ...
    zone = st.selectbox("वितरण केंद्र चुनें", hierarchy_df[hierarchy_df["Division"] == division]["Zone"].unique())
    ...
    msn_auto = st.selectbox("मीटर सीरियल नंबर फीडर (MSN) चुनें", hierarchy_df[hierarchy_df["Dtr code"] == dtr_code]["Msn"].unique())
...
# --------- Simple Scroll Time Picker ----------
def simple_time_picker(label):
    st.write(f"### {label}")
    col1, col2, col3 = st.columns([1,1,1])
    with col1:
        hour = st.selectbox("Hour", [f"{i:02d}" for i in range(1,13)], key=label+"_hour")
    with col2:
        minute = st.selectbox("Minute", [f"{i:02d}" for i in range(0,60)], key=label+"_minute")
    with col3:
        am_pm = st.selectbox("AM/PM", ["AM","PM"], key=label+"_ampm")
    return f"{hour}:{minute} {am_pm}"

# Date & Time inputs (only when MSN confirmed)
if final_msn:
    dtr_off_time = simple_time_picker("डीटीआर बंद करने का समय")
    dtr_on_time = simple_time_picker("डीटीआर चालू करने का समय")
